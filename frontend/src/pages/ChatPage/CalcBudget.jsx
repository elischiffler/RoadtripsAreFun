...
export const calcHotelBudget = async (route_duration, stops) => {
  ...
  if (duration < 25200) {
    // Can edit this number (set to 7 hours driving per day)
    return 0;
  } else {
    while (duration > 25200) {
      // Can edit this number (set to 7 hours driving per day)
      hotelCount++;
...
export const calcGasBudget = async (
  distanceInMeters,
  year,
  make,
  model,
  ...
  try {
    ...
    if (error.response?.status === 500) {
      ...
      addMessage(
        chatId,
        setChats,
        'We had an error getting your car information. Could you please re-enter your car information?',
        'bot'
      );
    }
...
const handleElectricCar = async (chatId, setChats) => {
  addMessage(
    chatId,
    setChats,
    'We currently do not support creating a traveling budget with electric cars. Would you like to skip creating a gas budget or input a new car?',
    'bot'
  );
};
