...
def find_google_hotels(
    ...
    if response.status_code == 200:
        listings = _parse_google_response(
            response.text
        )  # Convert the response to a string to parse
        if len(listings) > 0:  # Ensure listings were found
            # Filter hotels out of budget
            valid_hotels = list(
                filter(
                    lambda listing: price_range[0] <= listing["price"] <= price_range[1], listings
                )
            )
            ...
            while attempts > 0:  # Search through all hotel offerings
                ...
                if (
                    ideal_hotel is not None
                    and geodesic(
                        (ideal_hotel["coordinates"][0], ideal_hotel["coordinates"][1]), coords
                    ).miles
                    <= radius
                ):
                    return ideal_hotel
        ...
        raise HTTPException(
            status_code=404, detail="No valid hotels found for the given parameters"
        )  # To handle research
...
def _parse_google_response(response: str) -> List[Dict[str, Any]]:
    ...
    hotels_list = parser.xpath(
        "//div[@jsname='mutHjb']"
    )  # Get a list of divs which contain the specified jsname
    listings = []
    for hotel in hotels_list:
        name = hotel.xpath(
            ".//h2[@class='BgYkof ogfYpf ykx2he']/text()"
        )  # Get this classes text value
        google_url = hotel.xpath(
            ".//a[@class='PVOOXe']/@href"
        )  # Get the google listing link for later scraping
        ...
        if len(pricing_details) == 4:  # Ensure all pricing info is being returned
            price = int(
                "".join(filter(str.isdigit, pricing_details[0]))
            )  # Convert representation to an integer representation of price
            stars, review_count = _str_to_rating(rank_details[0])  # Extract rank information
            listings.append(
                {  # Add a hotel with all its relevant information
                    "name": name[0],
    ...
    if response.status_code == 200:
        ...
        if len(details) > 0:  # Ensure details were found
            ...
            hotel_location_path = details.xpath(".//div[@class='K4nuhf']")[
                0
            ]  # Exact container that will always have location
            address = hotel_location_path.xpath(".//span[@class='CFH2De']/text()")[
                0
            ]  # Get the full address from the website page
            location = get_location(
                geocoder=geolocator, address=address
            )  # Geolocate for additional area info
            coordinates = [
                location.latitude,
                location.longitude,
            ]  # Get the coordinates of the hotel
...
def _str_to_rating(rating: str) -> Tuple[float, int]:
    ...
    review_count = int(
        details[6].replace(",", "")
    )  # Remove commas to get review count as an integer
