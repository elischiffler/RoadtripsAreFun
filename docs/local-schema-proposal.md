...
**Status: proposed for review; not provisioned.** This is the schema supported by the checked-in code. It is not a claim about the live Neon database, which was not inspected or contacted. Do not apply this to production or any persistent local volume until the table definitions, existing data, and migration path are reviewed.
...
Before provisioning, review the route and leg identifier scope: `chat_crud.py` derives `route_id` from user and chat IDs and queries segments/steps by route or leg ID alone. Those identifiers and the proposed primary keys are therefore a data-isolation dependency. The repository has no foreign keys or cleanup of route segments, steps, and memory when a chat is deleted. These are existing behavior and need a separate migration/design review before adding constraints or changing keys. A clean local database can test this proposal; it cannot validate the shape or contents of production data.
